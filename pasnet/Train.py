...
def trainPASNet(train_x, train_y, eval_x, eval_y, pathway_mask, \
	...
	for epoch in range(nEpochs+1):
		...
		if epoch % 1000 == 0: 
			...
			train_auc = roc_auc_score(train_y.cpu().detach().numpy(), train_pred.cpu().detach().numpy())
			eval_auc = roc_auc_score(eval_y.cpu().detach().numpy(), eval_pred.cpu().detach().numpy())
			print("-----Epoch: ", epoch)
			print("Loss in Train: ", train_loss, "Loss in Test: ", eval_loss, "AUC in Train: ", train_auc, "AUC in Test: ", eval_auc)
